...
export function rankerAgent(): Agent {
  return {
    ...
    systemPrompt: [
      ...
      '1–5: each entry is the player name in bold with team + tier, then a COMPARATIVE take — why they',
      'rank above the next player (or the gap up to the one above) in fantasy terms (floor vs ceiling,',
      'target share / volume, ADP value, boom-or-bust) — and cite the source URL(s) from that player\'s',
      'note.',
